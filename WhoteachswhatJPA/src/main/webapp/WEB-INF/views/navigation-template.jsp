<!-- Navigation Starts -->
<div class="nav-fixed-left">
	<ul class="nav nav-side-menu">
		<li class="shadow-layer"></li>
		<li>
			<a href="welcome.html" >
				<i class="icon-photon home"></i>
				<span class="nav-selection">Dashboard</span>
			</a>
		</li>
		<li>
			<a href="#" >
				<i class="icon-photon layers_alt"></i>
				<span class="nav-selection">Admin Panel</span>
				<i class="icon-menu-arrow"></i>
			</a>
			<div class="sub-nav">
				<ul class="nav">
					<li>
						<a href="manageUsers">Manage Users</a>
					</li>
					<li>
						<a href="manageCourseSection">Manage Course Sections</a>
					</li>
				</ul>
			</div>
		</li>
		<li>
			<a href="#" >
				<i class="icon-photon new_window"></i>
				<span class="nav-selection">Manage Faculty</span>
				<i class="icon-menu-arrow"></i>
			</a>
			<div class="sub-nav">
				<ul class="nav">
					<li>
						<a href="addFaculty">Add</a>
					</li>
					<li>
						<a href="updateFaculty">Update</a>
					</li>
					<li>
						<a href="viewFaculty">View</a>
					</li>
				</ul>
			</div>
		</li>
		<li>
			<a href="#">
				<i class="icon-photon book_alt2"></i>
				<span class="nav-selection">Manage Course</span>
				<i class="icon-menu-arrow"></i>   
			</a>
			<div class="sub-nav">
				<ul class="nav">
					<li>
						<a href="addCourse">Add</a>
					</li>
					<li>
						<a href="updateCourse">Update</a>
					</li>
					<li>
						<a href="viewCourse">View</a>
					</li>
				</ul>
			</div>
		</li>
		<li>
			<a href="#">
				<i class="icon-photon book"></i>
				<span class="nav-selection">Manage Program</span>
				<i class="icon-menu-arrow"></i>   
			</a>
			<div class="sub-nav">
				<ul class="nav">
					<li>
						<a href="addProgram">Add</a>
					</li>
					<li>
						<a href="updateProgram">Update</a>
					</li>
					<li>
						<a href="viewProgram">View</a>
					</li>
				</ul>
			</div>
		</li>

		<li>
			<a href="#">
				<i class="icon-photon clock"></i>
				<span class="nav-selection">Manage Comp Hours</span>
				<i class="icon-menu-arrow"></i>   
			</a>
			<div class="sub-nav">
				<ul class="nav">
					<li>
						<a href="updateCompHourType">Update</a>
					</li>
					<li>
						<a href="manageCompHours">View</a>
					</li>
				</ul>
			</div>
		</li>
		
		<li>
			<a href="#">
				<i class="icon-photon layers"></i>
				<span class="nav-selection">Manage Responsibilities</span>
				<i class="icon-menu-arrow"></i>   
			</a>
			<div class="sub-nav">
				<ul class="nav">
					<li>
						<a href="updateResponsibility">Update</a>
					</li>
					<li>
						<a href="viewResponsibility">View</a>
					</li>
				</ul>
			</div>
		</li>
		
		<li>
			<a href="#">
				<i class="icon-photon chart_alt"></i>
				<span class="nav-selection">Reports</span>
				<i class="icon-menu-arrow"></i>   
			</a>
			<div class="sub-nav">
				<ul class="nav">
					<li>
						<a href="whoTeachesWhat">Who Teaches What</a>
					</li>
					<li>
						<a href="coursesOfferingNextSemester">Courses Offered Next Semester</a>
					</li>
					<li>
						<a href="teachingHistory">Teaching History</a>
					</li>
				</ul>
			</div>
		</li>
		<li>
			<a href="help" >
				<i class="icon-photon info"></i>
				<span class="nav-selection">Help</span>
				<!--TODO: Create documentation page -->
		
			</a>
		</li>
		<li class="nav-logout">
			<a href="logout">
				<!--TODO: Create logout webservice -->
				<i class="icon-photon key_stroke"></i><span class="nav-selection">Logout</span>
			</a>
		</li>
	</ul>
</div>   
<!-- Navigation ends -->     
